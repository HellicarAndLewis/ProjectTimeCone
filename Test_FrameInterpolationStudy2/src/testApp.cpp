...
void testApp::setup(){
	...
	this->controls.addSlider("Window size", 1, 100, &this->windowSize);
	...
	meshPanel->onMouse.addListener([this] (MouseArguments& args) {
		if (args.isLocal() && (args.action == MouseArguments::Dragged || args.action == MouseArguments::Pressed)) {
			this->interpolatePosition = args.localNormalised.x;
...
//--------------------------------------------------------------
void testApp::updateInterpolation() {
	//if this gets called before we init the mesh
	if (this->pointCloud.getVertices().size() == 0) {
		return;
	}
	...
	for (int j=0; j<this->height; j++) {
		for(int i=0; i<this->width; i++) {
			*vertices++ = ofVec3f(i, j, 0) + *displacement++ * this->interpolatePosition;
		}
	}
}